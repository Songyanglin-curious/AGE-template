...
- 撰写 `docs/references/directory-guide.md`——全仓库目录导航指南（名词对照表 + 快速决策矩阵 + 四层模型分区说明 + 23 个目录 + 根文件）
- 撰写 `docs/notes/common-scenarios-directory-map.md`——常见问题场景 → 目录映射（性能故障、需求设计、Bug 教训、交付回顾、学习实验、日常运转 6 类场景 + 覆盖盲区 + 判断口诀）
